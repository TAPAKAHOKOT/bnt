...
static constexpr float PLAYBACK_GAIN = 1.0f;
```
...
to signed 16-bit, so very loud TTS passages may distort above unity gain; it is
set to `1.0` (unity — no extra amplification), which stays clean.
